...
export default function UserProfile({ user, onUpdateProfile, stats }: UserProfileProps) {
	...
	const pickImage = async (source: 'camera' | 'gallery') => {
		try {
			...
			if (status !== 'granted') {
				Alert.alert('Permission Required', `We need ${source} access to update your avatar.`);
				return;
			}
			...
			<View className="px-6 space-y-4">
				...
				<View className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
					...
					<View className="flex-row justify-between">
						<View className={`items-center p-3 rounded-lg ${
